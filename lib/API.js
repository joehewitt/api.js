...
API.prototype = {
	route: function(server, middleware) {
		var pattern = this.apiPath + '/' + '*';

		var handler = _.bind(function(req, res) {
			var URL = url.parse(req.url);
			this.lookup(req.method, URL.pathname, _.bind(function(err, found) {
				if (err) {
					sendError(req, res, err, err ? err.error : 0);
				} else {
					this.invoke(found.methodDef, found.params, req.headers, req.query, req.params,
					_.bind(function(err, result) {
						if (err) {
							...
							if (typeof(result) == "object") {
								...
								if (typeof(finalResult.body) == "object") {
		...
		var args = middleware ? middleware.slice() : [];
		args.splice(0, 0, pattern);
		args.push(handler);

		server.all.apply(server, args);
	},

	call: function(method, pathname, headers, query, params, cb) {
		this.lookup(method, pathname, _.bind(function(err, found) {
			if (err) {
				cb(err);
			} else {
				this.invoke(found.methodDef, found.params, headers, query, params, cb);
			}
		}, this));
	},
	...
	invoke: function(methodDef, urlParams, headers, query, params, cb) {
		var args = [];
...
API.statics = {
	...
	POST: function(name, docs, fn) {
		return this.define("POSTS", name, docs, fn);
	},
